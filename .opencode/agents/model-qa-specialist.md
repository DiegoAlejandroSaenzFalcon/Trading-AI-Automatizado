...
def compute_psi(expected: pd.Series, actual: pd.Series, bins: int = 10) -> float:
    ...
    Interpretation:
      < 0.10  â†’ No significant shift (green)
      0.10â€“0.25 â†’ Moderate shift, investigation recommended (amber)
      >= 0.25 â†’ Significant shift, action required (red)
    """
...
def pdp_interaction(
    ...
    ax.set_title(f"PDP Interaction - {feature_pair[0]} Ã— {feature_pair[1]}")
    fig.tight_layout()
    fig.savefig(
    ...
    for var in variables:
        for period in periods[1:]:
            ...
            results.append({
                ...
                "period": period,
                "psi": psi,
                "flag": "ðŸ”´" if psi >= psi_threshold else (
                    "ðŸŸ¡" if psi >= 0.10 else "ðŸŸ¢"
                ),
            })
