//+------------------------------------------------------------------+
//|                    Pure_Fractal_FVG_Fusion_v11.0.mq5             |
//|   v11.0 — Paquete de mejoras derivado de la auditoria profunda   |
//|   de v10.12 (M15 XAUUSD, cuentas Cent/Standard/Micro).           |
//|                                                                    |
//|   CAMBIOS vs v10.12:                                              |
//|   1) DETECCION AUTOMATICA DE TIPO DE CUENTA (Cent/Standard/       |
//|      Micro): InpAccountType=AUTO detecta por moneda (USC/EUC...)  |
//|      y por relacion contrato/tick-value. Todos los inputs en USD  |
//|      (riesgo por trade, limite diario) se escalan internamente a  |
//|      unidades de cuenta; el mismo .set sirve para Cent y Standard|
//|      sin tocar nada.                                              |
//|   2) SENAL KALMAN BALANCEADA (modo 1): terminos normalizados por |
//|      barra (velocidad en ATR/bar, aceleracion en ATR/bar^2) con  |
//|      divisores EWMA adaptativos (z-score). Corrige el hallazgo   |
//|      S1 de la auditoria: la aceleracion ya NO puede dominar el   |
//|      sigmoide por unidades inconsistentes (antes |a_hat|*10      |
//|      aportaba hasta +-30 al argumento vs +-2.5 de la velocidad). |
//|      Modo 0 = legado v10.12 (disponible para comparacion).       |
//|   3) ENTRADAS CON CONFIRMACION DE VELA: InpBarConfirmEntry       |
//|      (default true) usa la probabilidad de la ultima vela        |
//|      CERRADA (no tick intrabarra) + InpMinBarsBetweenEntries     |
//|      (espera minimo N velas entre entradas).                     |
//|   4) AKF POR INNOVACION (InpAKFMode=1): R adaptativa con         |
//|      innovaciones del filtro (Mehra-lite: R = C_y - P_pred) en   |
//|      vez de la varianza de ticks; R sube SOLO cuando el modelo   |
//|      se equivoca, no cuando el mercado se mueve rapido.          |
//|   5) TRAILING/BE CON PISO DE SPREAD: la distancia de trailing y  |
//|      el lock de breakeven nunca quedan por debajo de             |
//|      InpTrailSpreadFloorMult x spread promedio (evita asfixia    |
//|      por spread en spikes; hallazgo S3 de la auditoria).         |
//|   6) VALIDACION DE STOPS (SYMBOL_TRADE_STOPS_LEVEL / FREEZE) en  |
//|      entrada, BE y trailing; retcodes de modify logueados (no    |
//|      fallos silenciosos).                                        |
//|   7) FILTROS OPCIONALES: InpRequireZoneProximity (entrada solo   |
//|      cerca de zona FVG activa), InpRegimeFilterEnable (bloquea   |
//|      entradas en volatilidad baja comparada con su media).       |
//|   8) PARCIALES OPCIONALES: InpPartialTPEnable cierra un % de la  |
//|      posicion en InpPartialTriggerATR (fijar ganancias, dejar    |
//|      correr el resto).                                           |
//|   9) PERSISTENCIA DE ESTADO: limite diario y cooldown se guardan |
//|      en GlobalVariables del terminal (sobreviven reinicios).     |
//|  10) PANEL: nueva linea de cuenta/tipo/senal + P/L del dia en    |
//|      USD reales; macro bias ahora acepta timeframe propio        |
//|      (InpMacroTimeframe, util H1/H4 para el sesgo).              |
//|                                                                    |
//|   ADVERTENCIA (misma que v10.12): esto reduce vectores de falla  |
//|   identificados; NO es promesa de rentabilidad. Valida en DEMO   |
//|   primero y luego en cuenta CENT real, con muestra suficiente.   |
//+------------------------------------------------------------------+
#property copyright "APEXQUANT / NeurAlgo project"
#property version   "11.0"
#property strict
#property description "Fusion Kalman + FVG/PA v11 — cuenta auto-detect (Cent/Std), senal balanceada, entradas con confirmacion de vela, trailing con piso de spread"
...
input int               InpMacroTimeframe = PERIOD_CURRENT; // TF del sesgo macro (0=grafico)
...
input bool              InpRequireZoneProximity = false; // Exigir precio cerca de zona FVG activa para entrar
...
input int               InpRegimeATRWindow      = 100;   // Velas de la media ATR del regimen
...
input double InpRiskPerTradeUSD  = 5.0;   // Riesgo por operacion EN USD (se escala solo a la cuenta)
...
input double InpTrailSpreadFloorMult  = 1.5;   // Piso de trailing/BE en x spread promedio (evita asfixia por spread)

input group "=== Controles Macro: Circuit Breaker Diario y Time-Stop por Volatilidad ==="
input bool   InpDailyLossLimitEnable = true;
input double InpMaxDailyLossUSD      = 30.0;  // Perdida REALIZADA maxima del dia EN USD -> bloquea nuevas entradas
...
int    g_hATR;
double g_bufATR[];
input ulong  InpMagicBase = 104; // MAGIC UNICO de esta version (asignable en .set)
...
//===================================================================
string GV_DayStart(){ return("PFVG11_"+_Symbol+"_"+IntegerToString(InpMagicBase)+"_dayStart"); }
string GV_DayPL()   { return("PFVG11_"+_Symbol+"_"+IntegerToString(InpMagicBase)+"_dayPL"); }
string GV_Cooldown(){ return("PFVG11_"+_Symbol+"_"+IntegerToString(InpMagicBase)+"_cooldown"); }
...
int OnInit()
{
    ...
    PrintFormat("Pure_Fractal_FVG_Fusion v11.0 initialised | Cuenta=%s (1 unidad = %.3f USD) | Senal=%s | AKF=%s | Entradas=%s | MacroTF=%s",
...
double ComputeBiasProbability(double mid, double atr)
{
    ...
    {
        //--- Modo balanceado (recomendado v11): terminos normalizados por
...
void CreateDashboard()
  {
   ...
   ObjectSetString(0, "DASH_Title", OBJPROP_TEXT, "Pure_Fractal FVG Fusion v11.0");
