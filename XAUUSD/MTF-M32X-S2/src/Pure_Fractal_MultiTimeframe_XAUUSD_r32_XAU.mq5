//+------------------------------------------------------------------+
//| Pure_Fractal_MultiTimeframe_XAUUSD_r32_XAU.mq5                   |
...
input double InpRiskPctEquity          = 1.2;            // [r32] % Riesgo por trade sobre Equity
input double InpRiskPerTradeUSD        = 20.0;          // Riesgo por trade en USD (si InpUseRiskPctEquity = false)
...
input double InpMaxDailyLossPct        = 4.0;            // [r32] Max Loss Diario en % de Equity
input double InpMaxDailyLossUSD        = 200.0;         // Max Loss Diario en USD (si InpUseRiskPctEquity = false)
input int    InpCooldownMinutes        = 0;              // Pausa global tras trade perdedor (0=off)

input group "=== Slot 01: 00-02h MOMEMA (validado) ==="
...
input group "=== Slot 02: 02-04h MOMEMA (validado) ==="
...
input group "=== Slot 03: 04-06h BREAK48 (validado) ==="
...
input group "=== Slot 04: 06-08h BREAK48 (validado) ==="
...
input group "=== Slot 05: 10-12h RETEST48 (validado) ==="
...
input group "=== Slot 06: OFF (EMACROSS filtrado por DD) ==="
...
input group "=== Slot 07: 16-18h RETEST48 (validado) ==="
...
input group "=== Slot 08: 22-24h BREAK48 (validado) ==="
...
input long   InpMagicBase    = 915001;
...
void TryEntries()
{
   ...
   for(int s = 0; s < MAX_SLOTS; s++)
   {
      ...
      bool ok = (sig == 1) ?
         g_trade.Buy(lots, _Symbol, 0.0, slPrice, tpPrice, "M32X_S" + IntegerToString(s + 1)) :
         g_trade.Sell(lots, _Symbol, 0.0, slPrice, tpPrice, "M32X_S" + IntegerToString(s + 1));
      if(ok)
      {
...
void SavePersistedState()
{
   ...
   GlobalVariableSet("M32X_DAYSTART", (double)g_dayStart);
   GlobalVariableSet("M32X_DAYPL", g_dayRealizedPL);
   GlobalVariableSet("M32X_COOLDOWN", (double)g_cooldownUntil);
}
void LoadPersistedState()
{
   ...
   datetime dayStart = now - (dtNow.hour * 3600 + dtNow.min * 60 + dtNow.sec);
   if(GlobalVariableCheck("M32X_COOLDOWN"))
   {
      datetime cd = (datetime)GlobalVariableGet("M32X_COOLDOWN");
      if(cd > now) g_cooldownUntil = cd;
   }
   if(GlobalVariableCheck("M32X_DAYSTART"))
   {
      datetime stored = (datetime)GlobalVariableGet("M32X_DAYSTART");
      if(stored == dayStart)
      {
         g_dayStart = dayStart;
         g_dayRealizedPL = GlobalVariableGet("M32X_DAYPL");
         double maxLossUSD = InpMaxDailyLossUSD;
         if(InpUseRiskPctEquity)
         {
            double equity = AccountInfoDouble(ACCOUNT_EQUITY);
