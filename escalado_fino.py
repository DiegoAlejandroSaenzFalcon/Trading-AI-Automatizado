...
def stats(trades, label):
    ...
    print(f"{label}: pnl={tot:>9.1f}  win={w:>5.1f}%  PF={pf:>5.2f}  maxDD={maxdd:>8.1f}  lote max={max(lots):.2f}  lote med={sum(lots)/len(lots):.2f}")

frm2 = dtm.datetime(2026, 5, 14); to2 = dtm.datetime(2026, 8, 11)
print("=== MAY-AGO (out of sample, realista) ===")
for r in (150, 300, 400, 500):
