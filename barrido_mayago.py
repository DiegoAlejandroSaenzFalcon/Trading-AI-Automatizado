    ...
    for k in range(len(m1)):
        ...
        if br.s.position is not None:
            ...
            if res is not None:
                ...
                if not br.s.dailyLimitHit and br.s.dayRealizedPL <= -120.0:
...
def ventana(h0, h1):
    ...
    br.INP["StartHour"] = h0; br.INP["StartMinute"] = 0
    br.INP["EndHour"] = h1; br.INP["EndMinute"] = 0

print("=== BARRIDO MAY-AGO (14 May - 11 Ago 2026, fuera de muestra) ===")
frm = dtm.datetime(2026, 5, 14)
to = dtm.datetime(2026, 8, 11)
cands = [(10,15),(12,16),(12,15),(11,15),(11,16),(10,16),(13,17),(14,18),(16,19),(16,20),(21,2),(22,2),(19,23),(0,3),(6,9)]
print(f"{'Ventana':<10}{'PnL':>10}{'Trades':>8}{'Win%':>8}")
for h0, h1 in cands:
    ventana(h0, h1)
    trades = run_period(True, frm, to) or []
    tot = sum(t["pnl"] for t in trades)
    n = len(trades)
    w = sum(1 for t in trades if t["pnl"]>0)/n*100 if n else 0
    print(f"{h0:02d}-{h1:02d}h    {tot:>10.1f}{n:>8}{w:>7.1f}%")
mt5.shutdown()
