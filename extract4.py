...
def extract(name_end, name_start, max_lines=300):
    out = []
    for l in lines:
        out.append(l)
    return "\n".join(lines)

def get(start_idx, max_lines=300):
    depth = 0
    out = []
...
for fn in ["void DetectNewFVG()", "void UpdatePriceActionFVGBias()", "void ExecuteBiasedEntry(", "void ManageExitsAndProtection(", "void ComputeAdaptiveSLTP(", "void IsWithinSession()", "void UpdateMacroFilter()"]:
    i = find(fn)
