...
input int               InpMacroTimeframe = PERIOD_H1; // TF del sesgo macro (0=grafico, H1 recomendado ORO)
...
input double InpRiskPerTradeUSD  = 20.0;  // Riesgo por operacion EN USD (ganador del barrido de sizing)
...
input double InpTPMultiplier     = 3.0;   // TP base = ATR x esto (R:R 2.5 - optimizado M30/H1)
...
input double InpMaxDailyLossUSD      = 80.0;  // Perdida REALIZADA maxima del dia EN USD (4x riesgo) -> bloquea nuevas entradas
...
input group "=== Filtro de Sesion (mejor ventana segun estudio ORO: NY 15-17h) ==="
...
double g_bufATR[];
input ulong  InpMagicBase = 31; // unico: r30B usa 30 // MAGIC UNICO de esta version (asignable en .set)
...
//===================================================================
string GV_DayStart(){ return("PFVG30X_"+_Symbol+"_"+IntegerToString(InpMagicBase)+"_dayStart"); }
string GV_DayPL()   { return("PFVG30X_"+_Symbol+"_"+IntegerToString(InpMagicBase)+"_dayPL"); }
string GV_Cooldown(){ return("PFVG30X_"+_Symbol+"_"+IntegerToString(InpMagicBase)+"_cooldown"); }
...
void DetectAccountType()
{
    ...
    if(centByCurrency) heuristic = 2;  // la moneda USC/EUC... manda: es cuenta CENT aunque la heuristica de tick value diga standard (XAUUSDm en cent: ratio~1.0)
   ...
   if(goLong)
     {
      ...
      sent = trade.Buy(lots, _Symbol, ask, sl, tp, usedConfluence ? "R30X+FVG Buy" : "R30X Buy");
      entryPrice = ask;
     }
   else
     {
      ...
      sent = trade.Sell(lots, _Symbol, bid, sl, tp, usedConfluence ? "R30X+FVG Sell" : "R30X Sell");
      entryPrice = bid;
     }
