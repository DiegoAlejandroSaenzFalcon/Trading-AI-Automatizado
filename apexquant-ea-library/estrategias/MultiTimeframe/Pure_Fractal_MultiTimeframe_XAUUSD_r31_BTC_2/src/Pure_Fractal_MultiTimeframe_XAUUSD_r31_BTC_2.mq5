...
input group "=== Slot 01: OFF (solo slots validados por sim) ==="
...
input group "=== Slot 04: OFF ==="
input ENUM_SLOT_STRATEGY InpSlot04Strategy = STRAT_OFF;
input int  InpSlot04Start = 0;
input int  InpSlot04End   = 24;
input double InpSlot04TP  = 2.0;
...
input long   InpMagicBase    = 910201;
...
void TryEntries()
{
   ...
   for(int s = 0; s < MAX_SLOTS; s++)
   {
      ...
      bool ok = (sig == 1) ?
         g_trade.Buy(lots, _Symbol, 0.0, slPrice, tpPrice, "M31C_S" + IntegerToString(s + 1)) :
         g_trade.Sell(lots, _Symbol, 0.0, slPrice, tpPrice, "M31C_S" + IntegerToString(s + 1));
      if(ok)
      {
...
void SavePersistedState()
{
   ...
   GlobalVariableSet("M31C_"+_Symbol+"_DAYSTART", (double)g_dayStart);
   GlobalVariableSet("M31C_"+_Symbol+"_DAYPL", g_dayRealizedPL);
   GlobalVariableSet("M31C_"+_Symbol+"_COOLDOWN", (double)g_cooldownUntil);
}
void LoadPersistedState()
{
   ...
   datetime dayStart = now - (dtNow.hour * 3600 + dtNow.min * 60 + dtNow.sec);
   if(GlobalVariableCheck("M31C_"+_Symbol+"_COOLDOWN"))
   {
      datetime cd = (datetime)GlobalVariableGet("M31C_"+_Symbol+"_COOLDOWN");
      if(cd > now) g_cooldownUntil = cd;
   }
   if(GlobalVariableCheck("M31C_"+_Symbol+"_DAYSTART"))
   {
      datetime stored = (datetime)GlobalVariableGet("M31C_"+_Symbol+"_DAYSTART");
      if(stored == dayStart)
      {
         g_dayStart = dayStart;
         g_dayRealizedPL = GlobalVariableGet("M31C_"+_Symbol+"_DAYPL");
