...
input double InpRiskPerTradeUSD        = 300.0;          // Riesgo por trade en USD (reales)
...
input double InpMaxDailyLossUSD        = 1200.0;
input int    InpCooldownMinutes        = 0;              // Pausa global tras trade perdedor (0=off)

input group "=== Slot 01: 04-06h VWAP ==="
input ENUM_SLOT_STRATEGY InpSlot01Strategy = STRAT_VWAP;
input int  InpSlot01Start = 4;
input int  InpSlot01End   = 6;
input double InpSlot01TP  = 2.5;
input int  InpSlot01TTL   = 48;

input group "=== Slot 02: 16-18h RETEST48 ==="
...
input group "=== Slot 03: 18-20h VWAP ==="
...
input group "=== Slot 04: 20-22h RETEST48 ==="
input ENUM_SLOT_STRATEGY InpSlot04Strategy = STRAT_RETEST48;
input int  InpSlot04Start = 20;
input int  InpSlot04End   = 22;
input double InpSlot04TP  = 2.5;
input int  InpSlot04TTL   = 48;

input group "=== Slot 05: OFF ==="
input ENUM_SLOT_STRATEGY InpSlot05Strategy = STRAT_OFF;
input int  InpSlot05Start = 0;
input int  InpSlot05End   = 24;
...
input long   InpMagicBase    = 910101;
...
void TryEntries()
{
   ...
   for(int s = 0; s < MAX_SLOTS; s++)
   {
      ...
      bool ok = (sig == 1) ?
         g_trade.Buy(lots, _Symbol, 0.0, slPrice, tpPrice, "M31B_S" + IntegerToString(s + 1)) :
         g_trade.Sell(lots, _Symbol, 0.0, slPrice, tpPrice, "M31B_S" + IntegerToString(s + 1));
      if(ok)
      {
...
void SavePersistedState()
{
   ...
   GlobalVariableSet("M31B_"+_Symbol+"_DAYSTART", (double)g_dayStart);
   GlobalVariableSet("M31B_"+_Symbol+"_DAYPL", g_dayRealizedPL);
   GlobalVariableSet("M31B_"+_Symbol+"_COOLDOWN", (double)g_cooldownUntil);
}
void LoadPersistedState()
{
   ...
   datetime dayStart = now - (dtNow.hour * 3600 + dtNow.min * 60 + dtNow.sec);
   if(GlobalVariableCheck("M31B_"+_Symbol+"_COOLDOWN"))
   {
      datetime cd = (datetime)GlobalVariableGet("M31B_"+_Symbol+"_COOLDOWN");
      if(cd > now) g_cooldownUntil = cd;
   }
   if(GlobalVariableCheck("M31B_"+_Symbol+"_DAYSTART"))
   {
      datetime stored = (datetime)GlobalVariableGet("M31B_"+_Symbol+"_DAYSTART");
      if(stored == dayStart)
      {
         g_dayStart = dayStart;
         g_dayRealizedPL = GlobalVariableGet("M31B_"+_Symbol+"_DAYPL");
