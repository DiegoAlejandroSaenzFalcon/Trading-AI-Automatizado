//+------------------------------------------------------------------+
//| Pure_Fractal_MultiTimeframe_XAUUSD_r31.mq5                       |
...
//| BTC: 04-06 VWAP2.5 | 16-18 RETEST482.0 | 18-20 VWAP2.0 |         |
//|      20-22 RETEST482.5  (riesgo 300 USD, via .set BTC)           |
...
input double InpRiskPerTradeUSD        = 20.0;           // Riesgo por trade en USD (reales)
input double InpMaxLotSize             = 2.0;
input int    InpSLMultiplierATR        = 80;             // SL = ATR_M5 x esto / 100 (0.80)
input double InpMaxSpreadATR           = 0.35;           // Filtro spread: bloquear si spread > ATR x esto (0=off)

input group "=== Circuit Breaker Diario ==="
input bool   InpDailyLossLimitEnable   = false;          // Bloquear entradas tras perdida diaria (default off = fiel al backtest)
input double InpMaxDailyLossUSD        = 200.0;
input int    InpCooldownMinutes        = 0;              // Pausa global tras trade perdedor (0=off)

input group "=== Slot 01: 00-02h ==="
input ENUM_SLOT_STRATEGY InpSlot01Strategy = STRAT_MOMEMA;
input int  InpSlot01Start = 0;
input int  InpSlot01End   = 2;
input double InpSlot01TP  = 3.0;
input int  InpSlot01TTL   = 48;

input group "=== Slot 02: 02-04h ==="
input ENUM_SLOT_STRATEGY InpSlot02Strategy = STRAT_MOMEMA;
input int  InpSlot02Start = 2;
input int  InpSlot02End   = 4;
input double InpSlot02TP  = 2.0;
input int  InpSlot02TTL   = 48;

input group "=== Slot 03: 04-06h (BREAK48 prioritario + MOMEMA) ==="
input ENUM_SLOT_STRATEGY InpSlot03Strategy = STRAT_BREAK48;
input bool  InpSlot03AltEnable = true;  // Usar MOMEMA como senal alternativa
input int  InpSlot03Start = 4;
input int  InpSlot03End   = 6;
input double InpSlot03TP  = 3.0;
input int  InpSlot03TTL   = 24;

input group "=== Slot 04: 06-08h ==="
input ENUM_SLOT_STRATEGY InpSlot04Strategy = STRAT_BREAK48;
input int  InpSlot04Start = 6;
input int  InpSlot04End   = 8;
input double InpSlot04TP  = 3.0;
input int  InpSlot04TTL   = 48;

input group "=== Slot 05: 10-12h ==="
input ENUM_SLOT_STRATEGY InpSlot05Strategy = STRAT_RETEST48;
input int  InpSlot05Start = 10;
input int  InpSlot05End   = 12;
input double InpSlot05TP  = 3.0;
input int  InpSlot05TTL   = 48;

input group "=== Slot 06: 14-16h ==="
input ENUM_SLOT_STRATEGY InpSlot06Strategy = STRAT_EMACROSS;
input int  InpSlot06Start = 14;
input int  InpSlot06End   = 16;
input double InpSlot06TP  = 2.0;
input int  InpSlot06TTL   = 48;

input group "=== Slot 07: 16-18h ==="
input ENUM_SLOT_STRATEGY InpSlot07Strategy = STRAT_RETEST48;
input int  InpSlot07Start = 16;
input int  InpSlot07End   = 18;
input double InpSlot07TP  = 2.0;
input int  InpSlot07TTL   = 48;

input group "=== Slot 08: 22-24h ==="
input ENUM_SLOT_STRATEGY InpSlot08Strategy = STRAT_BREAK48;
input int  InpSlot08Start = 22;
...
input long   InpMagicBase    = 910001;
...
void DetectAccountType()
{
   ...
   PrintFormat("[MULTI31] Cuenta detectada: %s (moneda=%s, ratio=%.2f, 1 unidad=%.4f USD)",
               g_acctName, cur, ratio, g_acctUnit);
   Print("[MULTI31] Si la deteccion es incorrecta, fija InpAccountType manualmente (1=Std, 2=Cent, 3=Micro).");
}
//+------------------------------------------------------------------+
//  Lotes
//+------------------------------------------------------------------+
...
void TryEntries()
{
   ...
   for(int s = 0; s < MAX_SLOTS; s++)
   {
      ...
      bool ok = (sig == 1) ?
         g_trade.Buy(lots, _Symbol, 0.0, slPrice, tpPrice, "M31X_S" + IntegerToString(s + 1)) :
         g_trade.Sell(lots, _Symbol, 0.0, slPrice, tpPrice, "M31X_S" + IntegerToString(s + 1));
      if(ok)
      {
         g_slots[s].ticket = g_trade.ResultOrder();
         g_slots[s].entryBarIdx = Bars(_Symbol, TF);
         PrintFormat("[MULTI31] %s SLOT%d %s lote=%.2f SL=%.2f TP=%.2f",
                        ...
                        const MqlTradeResult &result)
{
   ...
   if(InpDailyLossLimitEnable && !g_dailyLimitHit && g_dayRealizedPL <= -MathAbs(InpMaxDailyLossUSD))
   {
      ...
      Print("[MULTI31] Limite diario alcanzado: bloqueo de entradas por hoy.");
   }
}
//+------------------------------------------------------------------+
...
void SavePersistedState()
{
   ...
   GlobalVariableSet("M31X_"+_Symbol+"_DAYSTART", (double)g_dayStart);
   GlobalVariableSet("M31X_"+_Symbol+"_DAYPL", g_dayRealizedPL);
   GlobalVariableSet("M31X_"+_Symbol+"_COOLDOWN", (double)g_cooldownUntil);
}
void LoadPersistedState()
{
   ...
   datetime dayStart = now - (dtNow.hour * 3600 + dtNow.min * 60 + dtNow.sec);
   if(GlobalVariableCheck("M31X_"+_Symbol+"_COOLDOWN"))
   {
      datetime cd = (datetime)GlobalVariableGet("M31X_"+_Symbol+"_COOLDOWN");
      if(cd > now) g_cooldownUntil = cd;
   }
   if(GlobalVariableCheck("M31X_"+_Symbol+"_DAYSTART"))
   {
      datetime stored = (datetime)GlobalVariableGet("M31X_"+_Symbol+"_DAYSTART");
      if(stored == dayStart)
      {
         g_dayStart = dayStart;
         g_dayRealizedPL = GlobalVariableGet("M31X_"+_Symbol+"_DAYPL");
         if(InpDailyLossLimitEnable && g_dayRealizedPL <= -MathAbs(InpMaxDailyLossUSD))
            g_dailyLimitHit = true;
      }
   }
}
...
int OnInit()
{
   ...
   PrintFormat("[MULTI31] EA cargado: %s | Riesgo %.2f USD/trade | MaxLote %.2f | Slots activos: %d",
               _Symbol, InpRiskPerTradeUSD, InpMaxLotSize, 0);
