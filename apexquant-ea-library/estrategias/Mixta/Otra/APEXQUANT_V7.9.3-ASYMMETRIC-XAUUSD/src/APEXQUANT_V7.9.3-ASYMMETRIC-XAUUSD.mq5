//+------------------------------------------------------------------+
//|   APEXQUANT - V7.9.3-ASYMMETRIC-XAUUSD                            |
...
//|                                                                  |
//| V7.9.3 -- CAMBIO DE COMPORTAMIENTO EN STAGE1 (pedido explicito): |
//|   - Breathing room bajado de 120s a 60s (espera 1 minuto).       |
//|   - La 2da operacion tras la primaria ahora es SIEMPRE contraria |
//|     con lote MAYOR (Inp_HedgeRatio 0.50->2.50) para frenar el    |
//|     flotante negativo si el precio sigue en contra, en vez de    |
//|     abrir con lote menor manteniendo sesgo hacia la primaria.    |
//|   - El refuerzo direccional (misma direccion que la primaria)    |
//|     pasa a ser un modo opcional, OFF por defecto                 |
//|     (Inp_UseDirectionalStage true->false).                       |
//|   - Stage2 (3ra operacion en direccion del mercado) no cambio:   |
//|     ya hacia lo pedido.                                          |
//|                                                                  |
//| Ver comentarios "[CALIBRACION]" junto a cada input recalibrado   |
//| para el razonamiento y la cifra de origen en los datos.          |
//+------------------------------------------------------------------+
#property copyright "ApexQuant V7.9.3-XAU | XAUUSD Anti-Symmetric Engine"
#property version   "7.93"
#property strict
#property description "XAUUSD | V7.9.3-ASYMMETRIC | Directional Recovery | Anti-Symmetric Guard"

#define VERSION_STR   "APEXQUANT_V7.9.3-XAU"
...
bool AntiSymmetricOK(ENUM_ORDER_TYPE type, double lot)
{
   ...
   Print("[AQ V7.9.3-XAU] ANTI-SYMMETRIC GUARD: apertura bloqueada - crea simetria | NetVol=",
...
double MaxSafeLot(double proposedLot, ENUM_ORDER_TYPE type=ORDER_TYPE_BUY)
{
   ...
   if(testMargin>maxMarginAllowed&&maxMarginAllowed>0){
      ...
      if(scaledLot<capped){
         Print("[AQ V7.9.3-XAU] MaxSafeLot: recorte de ",capped," a ",scaledLot,
...
bool DailyLimitReached()
{
   ...
   if(eff<=-lim){Print("[AQ V7.9.3-XAU] LIMITE DIARIO");m_dailyLimitHit=true;m_isPaused=true;}
...
bool ClosePos(ulong ticket, string reason="")
{
   ...
   if(!m_isProcessing&&m_port.totalPos>1){
      Print("[AQ V7.9.3-XAU] !! CIERRE INDIVIDUAL BLOQUEADO #",ticket," [",reason,"] totalPos=",m_port.totalPos);
      return false;
   }
   double pf=PositionGetDouble(POSITION_PROFIT)+PositionGetDouble(POSITION_SWAP);
   if(m_trade.PositionClose(ticket)){
      ...
      if(idx>=0){
         ...
         Print("[AQ V7.9.3-XAU] CERRADA #",ticket," $",NormalizeDouble(pf,2),(reason!=""?" ["+reason+"]":""));
         ZeroMemory(m_rec[idx]);
      }
      return true;
   }
...
bool CloseRescuePos(ulong ticket, string reason)
{
   ...
   if(m_trade.PositionClose(ticket)){m_totalPnL+=pf;m_tradesClosed++;
      Print("[AQ V7.9.3-XAU] RESCATE #",ticket," $",NormalizeDouble(pf,2)," [",reason,"]"); return true;}
   return false;
}
...
bool CloseBlockIfPositive(string reason)
{
   ...
   Print("[AQ V7.9.3-XAU] CIERRE POSITIVO: PnL=$",NormalizeDouble(m_port.totalProfit,2),
...
ulong OpenOrder(ENUM_ORDER_TYPE type,double lot,string comment,bool skipPosLimit=false)
{
   ...
   if(!ok){Print("[AQ V7.9.3-XAU] ERR apertura: ",m_trade.ResultRetcodeDescription());return 0;}
   ulong ticket=m_trade.ResultOrder();
   if(ticket>0){
      m_tradesOpened++;
      Print("[AQ V7.9.3-XAU] ABIERTA #",ticket," ",(type==ORDER_TYPE_BUY?"BUY":"SELL"),
...
void RunDetangle()
{
   ...
   if(m_detangleDetectTime==0){
      ...
      Print("[AQ V7.9.3-XAU] DETANGLE: jaula simetrica detectada | NetVol=",
   ...
   if(m_port.worstTicket>0&&m_port.worstProfit<0){
      Print("[AQ V7.9.3-XAU] DETANGLE EJECUTANDO: cerrando peor pos #",m_port.worstTicket,
         ...
         if(Inp_UseDirectionalStage&&trendDir==primaryDir&&trendDir!=0){
            ...
            if(MarginOK(reinLot,reinType)&&AntiSymmetricOK(reinType,reinLot)){
               ...
               if(t1>0){
                  ...
                  Print("[AQ V7.9.3-XAU] >>> STAGE 2 via REFUERZO (modo opcional)...");
               }
            }
         } else {
            ...
            if(!AntiSymmetricOK(hedgeType, hedgeLot)) {
                ...
                Print("[AQ V7.9.3-XAU] CONTRARIA ADJUSTED: Forzando asimetria matemática. Nuevo lote: ", hedgeLot);
            }
            ...
            if(MarginOK(hedgeLot,hedgeType)){
               ...
               if(t1>0){
                  ...
                  m_stage3TriggerAtOpen=m_dyn.stage3Trigger;
                  Print("[AQ V7.9.3-XAU] >>> STAGE 2 via CONTRARIA DE FRENADO (lote ",DoubleToString(hedgeLot,2),"): #",t1);
               }
            }
         }
      }
   ...
   if(m_blockStage==2&&mainPosCount==2){
      ...
      if(MarginOK(thirdLot,thirdType)){
         ...
         if(t2>0){
            ...
            Print("[AQ V7.9.3-XAU] >>> STAGE 3: 3ra DIRECCIONAL #",t2,
      ...
      if(totalPnL<=trigger3){
         ...
         if(MarginOK(fourthLot,fourthType)){
            ...
            if(t3>0){
               ...
               m_blockStage=4;
               Print("[AQ V7.9.3-XAU] >>> STAGE 4: Consolidacion #",t3,
...
void RunRecoveryEngine()
{
   ...
   if(!m_recoveryActive){
      ...
      Print("[AQ V7.9.3-XAU] RECOVERY FALLBACK | PnL=$",NormalizeDouble(m_port.totalProfit,2));
   }
...
void ActivateLBC()
{
   ...
   Print("[AQ V7.9.3-XAU] LBC ACTIVADO | LibreMarg=$",NormalizeDouble(freeMarg,2)," | MaxPares=",m_lbc.maxOrdersCalc);
}

void DeactivateLBC()
{
   if(!m_lbc.active) return;
   Print("[AQ V7.9.3-XAU] LBC DESACTIVADO | Cosechado:$",NormalizeDouble(m_lbc.harvestedTotal,2)," en ",m_lbc.harvestCount," cosechas");
   ZeroMemory(m_lbc);
}
...
void CheckCycleMaxLoss()
{
   ...
   if(m_port.totalProfit<=Inp_CycleMaxLossUSD){
      Print("[AQ V7.9.3-XAU] CYCLE MAX LOSS: $",NormalizeDouble(m_port.totalProfit,2));
...
bool CheckEquityGuard()
{
   ...
   if(m_port.totalProfit<=Inp_EmergencyLossUSD&&!m_emergencyMode){
      Print("[AQ V7.9.3-XAU] ALERTA EQUITY: $",NormalizeDouble(m_port.totalProfit,2)," -> Pausa primarias.");
...
void RunCTEngine()
{
   ...
   if(m_port.totalPos==0){
      if(!m_sensors.allOK){
         ...
         if(TimeCurrent()-lastSL>=60){Print("[AQ V7.9.3-XAU] ENTRADA BLOQUEADA: ",m_sensors.blockReason);lastSL=TimeCurrent();}
         return;
      }
      ...
      if(ticket>0){
         ...
         m_detangleDetectTime=0;m_detangleActive=false;
         DeactivateLBC();
         Print("[AQ V7.9.3-XAU] PRIMARY #",ticket," ",(initType==ORDER_TYPE_BUY?"BUY":"SELL"));
      }
      m_isProcessing=false;
      return;
   }
...
void RunNetExposureHedge()
{
   ...
   if(loss<=m_dyn.netHedgeTrig2&&!m_netHedge2Applied){
      ...
      if(hedgeLot>0) {
         if(!AntiSymmetricOK(hedgeType, hedgeLot)) {
            hedgeLot = NormLot(hedgeLot + volStep);
            Print("[AQ V7.9.3-XAU] NET HEDGE L2 ADJUSTED to prevent 0.00 Net Volume.");
         }
         ...
         if(MarginOK_Hedge(hedgeLot,hedgeType)){
            ...
            if(ticket>0){m_netHedge2Applied=m_netHedge1Applied=true;m_lastNetHedgeTime=TimeCurrent();
               ...
               Print("[AQ V7.9.3-XAU] NET HEDGE L2 | PnL=$",NormalizeDouble(loss,2));}
         }
      }
      return;
   }
   ...
   if(loss<=m_dyn.netHedgeTrig1&&!m_netHedge1Applied){
      ...
      if(hedgeLot>0) {
         if(!AntiSymmetricOK(hedgeType, hedgeLot)) {
            hedgeLot = NormLot(hedgeLot + volStep);
            Print("[AQ V7.9.3-XAU] NET HEDGE L1 ADJUSTED to prevent 0.00 Net Volume.");
         }
         ...
         if(MarginOK_Hedge(hedgeLot,hedgeType)){
            ...
            if(ticket>0){m_netHedge1Applied=true;m_lastNetHedgeTime=TimeCurrent();
               ...
               Print("[AQ V7.9.3-XAU] NET HEDGE L1 | PnL=$",NormalizeDouble(loss,2));}
         }
      }
   }
}
...
void RunVolatilityStormFilter()
{
   ...
   if(stormNow&&!m_stormActive){m_stormActive=true;m_stormDetectedTime=TimeCurrent();Print("[AQ V7.9.3-XAU] TORMENTA | ATRx=",NormalizeDouble(m_stormLastATRRatio,2));}
   if(m_stormActive){if(TimeCurrent()-m_stormDetectedTime>=Inp_StormCooldownSec){if(!stormNow){m_stormActive=false;Print("[AQ V7.9.3-XAU] TORMENTA DESPEJADA");}else m_stormDetectedTime=TimeCurrent();}}
}
...
int OnInit()
{
   ...
   if(h_ATR==INVALID_HANDLE||h_EMAFast==INVALID_HANDLE||h_EMASlow==INVALID_HANDLE||h_RSI==INVALID_HANDLE||h_MACD==INVALID_HANDLE){Print("[AQ V7.9.3-XAU] ERROR: Indicadores base");return INIT_FAILED;}
   ...
   if(m_port.totalPos>0) Print("[AQ V7.9.3-XAU] Posiciones existentes (",m_port.totalPos,"): Stage fallback hasta nueva primaria.");
   ...
   Print("[AQ V7.9.3-XAU] LISTO | Saldo=$",m_initialBalance," | Marg0.01=$",NormalizeDouble(CalcMarginFor001(),2));
...
{
   Print("[AQ V7.9.3-XAU] DETENIDO | PnL=$",NormalizeDouble(m_totalPnL,2)," | Trades:",m_tradesClosed);
   ...
   if(id==CHARTEVENT_OBJECT_CLICK){
      if(sp=="AQ79_B1"){
         ...
         if(!m_isPaused){
            ...
            m_stage1TriggerAtOpen=m_stage3TriggerAtOpen=0;
            m_detangleDetectTime=0;m_detangleActive=false;m_primaryOpenTime=0;
            DeactivateLBC();Print("[AQ V7.9.3-XAU] SISTEMA REANUDADO");
         } else Print("[AQ V7.9.3-XAU] SISTEMA PAUSADO");
      }
      if(sp=="AQ79_B2"){
         Print("[AQ V7.9.3-XAU] CIERRE MANUAL...");int closed=0;
         m_isProcessing=true;
         for(int i=PositionsTotal()-1;i>=0;i--){
            ulong t=PositionGetTicket(i);if(!PositionSelectByTicket(t))continue;
            if(PositionGetString(POSITION_SYMBOL)!=_Symbol)continue;
         ...
         m_blockStage=0;m_stageFollowHedge=false;m_stage1TriggerAtOpen=m_stage3TriggerAtOpen=0;
         m_detangleDetectTime=0;m_detangleActive=false;m_primaryOpenTime=0;
         DeactivateLBC();Print("[AQ V7.9.3-XAU] CIERRE MANUAL: ",closed," posiciones cerradas");
      }
      ChartRedraw(0);
   }
}
