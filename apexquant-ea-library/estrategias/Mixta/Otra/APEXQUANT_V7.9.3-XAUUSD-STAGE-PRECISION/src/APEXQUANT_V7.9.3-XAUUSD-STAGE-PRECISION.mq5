//+------------------------------------------------------------------+
//|   APEXQUANT - V7.9.3-XAUUSD-STAGE-PRECISION                       |
//|   "STAGE 2-4 DE PRECISION -- CIERRE RAPIDO, ANTI-MARGIN-CALL"     |
//|                                                                  |
//| BASE: V7.9.3-ASYMMETRIC-XAUUSD (Stage1 = contraria de frenado,   |
//| identico, sin tocar -- ver esa version para el detalle completo  |
//| de la re-adaptacion BTC->XAUUSD).                                |
//|                                                                  |
//| ESTA VARIANTE SOLO TOCA STAGE 2, 3 Y 4:                          |
//|   - [PREC-1] CalcDirectionalLot() usa Inp_StageMoveATR (1.0x,    |
//|     antes reciclaba el 0.5x del Recovery Engine fallback) ->     |
//|     asume un movimiento mas realista para recuperar => lotes     |
//|     menores para la misma perdida acumulada.                     |
//|   - [PREC-2] Tope de crecimiento entre etapas (Inp_MaxStageLot-  |
//|     Growth): la 3ra/4ta operacion no puede superar Nx el lote de |
//|     la etapa anterior, leido via FindStageLotByComment() sin     |
//|     tocar Stage1. Frena la curva tipo martingala sin control.    |
//|   - [PREC-3] blockTP se reduce en Stage3 (Inp_Stage3TPFactor) y  |
//|     mas aun en Stage4 (Inp_Stage4TPFactor): mientras mas profundo|
//|     el bloque, menos exige de ganancia para cerrar -- prioriza   |
//|     cerrar rapido y con exposicion menor sobre el objetivo       |
//|     original completo. Piso de $0.15 para no perseguir un        |
//|     objetivo tan chico que ni cubra spread de 3-4 posiciones.    |
//|   - [PREC-4] RunNetExposureHedge() ahora respeta m_blockStage>0  |
//|     -- ya no abre su propia cobertura en paralelo con Stage1-4   |
//|     (hallazgo de auditoria: causaba aperturas duplicadas).       |
//|   - MaxSafeLot() (tope por % de margen libre) sigue aplicando    |
//|     como ultima red de seguridad en cada lote de Stage2-4.       |
//|                                                                  |
//| TRADE-OFF A TENER EN CUENTA: lotes mas chicos + objetivo mas     |
//| chico en Stage3/4 puede significar MENOS ganancia por ciclo      |
//| cuando el mercado SI coopera, a cambio de mucha menor exposicion |
//| cuando NO coopera. Valida en Strategy Tester contra la version   |
//| base V7.9.3-ASYMMETRIC-XAUUSD para comparar drawdown vs retorno. |
//+------------------------------------------------------------------+
#property copyright "ApexQuant V7.9.3-XAU-STGPRE | XAUUSD Stage2-4 Precision Engine"
#property version   "7.93"
#property strict
#property description "XAUUSD | V7.9.3-STAGE-PRECISION | Directional Recovery | Anti-Symmetric Guard"

#define VERSION_STR   "APEXQUANT_V7.9.3-XAU-STGPRE"
...
// ================================================================
//  [V7.9.3] BREATHING ROOM + CONTRARIA DE FRENADO
// ================================================================
input group "=== [V7.9.3] BREATHING ROOM ==="
// Segundos minimos que debe vivir la primaria antes de evaluar Stage1.
// No importa si va en contra unos segundos: se espera este tiempo a
// que el precio se mueva a favor antes de reaccionar.
input int    Inp_PrimaryMinHoldSec   = 60;
// Multiplicador del trigger normal para el override de emergencia
// (permite reaccionar ANTES de Inp_PrimaryMinHoldSec si la perdida ya
// es muy severa, sin esperar el minuto completo).
input double Inp_StageEmergMult      = 2.0;

input group "=== [V7.9.3] CONTRARIA DE FRENADO (Stage1) ==="
// [CAMBIO DE COMPORTAMIENTO] Al vencer Inp_PrimaryMinHoldSec, si el
// flotante sigue negativo mas alla del trigger (ver DYNAMIC THRESHOLD
// ENGINE / Inp_Stage1Trigger = "el valor en dolares configurable"),
// Stage1 abre SIEMPRE una segunda operacion CONTRARIA a la primaria,
// con lote MAYOR, para frenar el crecimiento del flotante negativo si
// el precio sigue en contra. Con Inp_HedgeRatio>1, el neto del grupo
// se recupera a una tasa proporcional a (Inp_HedgeRatio-1) por cada
// movimiento adicional en contra de la primaria.
// Ratio del lote de la contraria vs lote de la primaria en Stage1.
// 2.5 = contraria con 2.5x el lote de la primaria (recomendado >1.0;
// valores <=1.0 no frenan el flotante, solo lo diluyen o lo siguen).
input double Inp_HedgeRatio          = 2.5;
// OFF por defecto (tu comportamiento pedido: la 2da operacion es
// SIEMPRE contraria, sin condicionarla a tendencia). Si lo activas en
// true y TEMA+Kalman confirma tendencia A FAVOR de la primaria, refuerza
// en la misma direccion en vez de abrir la contraria (modo heredado,
// opcional).
input bool   Inp_UseDirectionalStage = false;
// Lote del refuerzo de primaria, solo si Inp_UseDirectionalStage=true
// (como multiplo del lote base)
...
input group "=== [V7.9.3] STAGE 2-4 PRECISION (evitar margin call) ==="
// Distancia de movimiento (en xATR) que asume CalcDirectionalLot() para
// dimensionar la 3ra/4ta operacion. Mas alto = asume que hace falta MAS
// movimiento a favor para recuperar => lote MENOR y mas conservador.
// (Antes se reciclaba Inp_RecoveryMoveATR=0.5, pensado para el motor de
// Recovery fallback, no para el BSE).
input double Inp_StageMoveATR        = 1.0;
// Tope de crecimiento del lote de una etapa vs el lote de la etapa
// anterior (ej. 3.0 = la 3ra operacion no puede superar 3x el lote de
// la contraria de Stage1, sin importar cuanto pida la formula). Frena
// la curva de crecimiento tipo martingala sin control.
input double Inp_MaxStageLotGrowth   = 3.0;
// Al llegar a Stage3/Stage4, el objetivo de cierre positivo (blockTP)
// se reduce a este % del original -- prioriza cerrar rapido y con
// exposicion menor sobre esperar la ganancia completa inicial.
input double Inp_Stage3TPFactor      = 0.50;
input double Inp_Stage4TPFactor      = 0.20;
...
void UpdateDynamicThresholds()
{
   ...
   // [V7.9.3-STAGE-PRECISION] Cuanto mas profundo el bloque (Stage3/4),
   // menos exigente el objetivo de cierre: prioriza cerrar rapido y con
   // exposicion menor sobre esperar la ganancia completa original. Un
   // piso en dolares evita perseguir un objetivo tan chico que ni cubra
   // spread/comisiones de las 3-4 posiciones abiertas.
   if(m_blockStage==3)      m_dyn.blockTP=MathMax(m_dyn.blockTP*Inp_Stage3TPFactor,0.15);
   else if(m_blockStage>=4) m_dyn.blockTP=MathMax(m_dyn.blockTP*Inp_Stage4TPFactor,0.15);
}
...
bool AntiSymmetricOK(ENUM_ORDER_TYPE type, double lot)
{
   ...
   Print("[AQ V7.9.3-XAU-STGPRE] ANTI-SYMMETRIC GUARD: apertura bloqueada - crea simetria | NetVol=",
...
double MaxSafeLot(double proposedLot, ENUM_ORDER_TYPE type=ORDER_TYPE_BUY)
{
   ...
   if(testMargin>maxMarginAllowed&&maxMarginAllowed>0){
      ...
      if(scaledLot<capped){
         Print("[AQ V7.9.3-XAU-STGPRE] MaxSafeLot: recorte de ",capped," a ",scaledLot,
...
bool DailyLimitReached()
{
   ...
   if(eff<=-lim){Print("[AQ V7.9.3-XAU-STGPRE] LIMITE DIARIO");m_dailyLimitHit=true;m_isPaused=true;}
...
bool ClosePos(ulong ticket, string reason="")
{
   ...
   if(!m_isProcessing&&m_port.totalPos>1){
      Print("[AQ V7.9.3-XAU-STGPRE] !! CIERRE INDIVIDUAL BLOQUEADO #",ticket," [",reason,"] totalPos=",m_port.totalPos);
      return false;
   }
   double pf=PositionGetDouble(POSITION_PROFIT)+PositionGetDouble(POSITION_SWAP);
   if(m_trade.PositionClose(ticket)){
      ...
      if(idx>=0){
         ...
         Print("[AQ V7.9.3-XAU-STGPRE] CERRADA #",ticket," $",NormalizeDouble(pf,2),(reason!=""?" ["+reason+"]":""));
         ZeroMemory(m_rec[idx]);
      }
      return true;
   }
...
bool CloseRescuePos(ulong ticket, string reason)
{
   ...
   if(m_trade.PositionClose(ticket)){m_totalPnL+=pf;m_tradesClosed++;
      Print("[AQ V7.9.3-XAU-STGPRE] RESCATE #",ticket," $",NormalizeDouble(pf,2)," [",reason,"]"); return true;}
   return false;
}
...
bool CloseBlockIfPositive(string reason)
{
   ...
   Print("[AQ V7.9.3-XAU-STGPRE] CIERRE POSITIVO: PnL=$",NormalizeDouble(m_port.totalProfit,2),
...
// [V7.9.3-STAGE-PRECISION] Busca el volumen de la posicion de la etapa
// anterior por su comentario (BSE_H1/BSE_REINF1 para Stage1->2,
// BSE_DIR_LONG/SHORT para Stage2->3). Permite capar el crecimiento entre
// etapas sin necesidad de tracking adicional ni de tocar el codigo de
// Stage1.
double FindStageLotByComment(string tag)
{
   for(int i=0;i<MAX_RECORDS;i++){
      if(m_rec[i].ticket!=0&&StringFind(m_rec[i].comment,tag)>=0) return m_rec[i].volume;
   }
   return 0;
}

// [V7.9.3-STAGE-PRECISION] Calcula el lote direccional de Stage2/Stage3.
// Cambios vs la version anterior (V7.9.2), con foco en evitar margin call:
//  1) Usa Inp_StageMoveATR (mas conservador, def. 1.0xATR) en vez de
//     reciclar Inp_RecoveryMoveATR (0.5xATR, pensado para el Recovery
//     Engine fallback). Asumir un movimiento mas realista para recuperar
//     produce lotes menores para la misma perdida acumulada.
//  2) prevStageLot (lote de la etapa anterior, via FindStageLotByComment)
//     limita el crecimiento a Inp_MaxStageLotGrowth veces ese lote, sin
//     importar cuanto pida la formula -- evita la curva de crecimiento
//     tipo martingala sin freno.
//  3) MaxSafeLot() (ya existente) sigue aplicando el tope por margen
//     libre como ultima red de seguridad.
double CalcDirectionalLot(int targetDir, double prevStageLot=0)
{
   ...
   double moveDist=atr*Inp_StageMoveATR; if(moveDist<=0) moveDist=atr*1.0;
   ...
   if(prevStageLot>0){
      double growthCap=prevStageLot*Inp_MaxStageLotGrowth;
      if(calcLot>growthCap){
         Print("[AQ V7.9.3-XAU-STGPRE] STAGE LOT CAPADO: formula pedia ",DoubleToString(calcLot,2),
               ", limitado a ",DoubleToString(growthCap,2)," (",DoubleToString(Inp_MaxStageLotGrowth,1),
               "x lote etapa anterior=",DoubleToString(prevStageLot,2),")");
         calcLot=growthCap;
      }
   }
...
ulong OpenOrder(ENUM_ORDER_TYPE type,double lot,string comment,bool skipPosLimit=false)
{
   ...
   if(!ok){Print("[AQ V7.9.3-XAU-STGPRE] ERR apertura: ",m_trade.ResultRetcodeDescription());return 0;}
   ulong ticket=m_trade.ResultOrder();
   if(ticket>0){
      m_tradesOpened++;
      Print("[AQ V7.9.3-XAU-STGPRE] ABIERTA #",ticket," ",(type==ORDER_TYPE_BUY?"BUY":"SELL"),
...
void RunDetangle()
{
   ...
   if(m_detangleDetectTime==0){
      ...
      Print("[AQ V7.9.3-XAU-STGPRE] DETANGLE: jaula simetrica detectada | NetVol=",
   ...
   if(m_port.worstTicket>0&&m_port.worstProfit<0){
      Print("[AQ V7.9.3-XAU-STGPRE] DETANGLE EJECUTANDO: cerrando peor pos #",m_port.worstTicket,
...
void RunBlockStageEngine()
{
   ...
   // ── STAGE 1: Primaria activa — espera Inp_PrimaryMinHoldSec y
   //    evalua el trigger en dolares (Inp_Stage1Trigger + motor dinamico) ─
   if(m_blockStage==1&&mainPosCount==1){
      ...
      if(totalPnL<=trigger1){
         ...
         if(Inp_UseDirectionalStage&&trendDir==primaryDir&&trendDir!=0){
            // REFUERZO (modo heredado/opcional, OFF por defecto): la tendencia
            // ya confirma a favor de la primaria, asi que se refuerza en la
            // misma direccion en vez de abrir la contraria.
            ...
            if(MarginOK(reinLot,reinType)&&AntiSymmetricOK(reinType,reinLot)){
               ...
               if(t1>0){
                  ...
                  Print("[AQ V7.9.3-XAU-STGPRE] >>> STAGE 2 via REFUERZO (modo opcional)...");
               }
            }
         } else {
            // CONTRARIA DE FRENADO (comportamiento por defecto solicitado):
            // siempre opuesta a la primaria, con Inp_HedgeRatio>1 -> lote
            // MAYOR que la primaria para frenar/revertir el crecimiento del
            // flotante negativo si el precio sigue en esa direccion.
            ...
            if(!AntiSymmetricOK(hedgeType, hedgeLot)) {
                ...
                Print("[AQ V7.9.3-XAU-STGPRE] CONTRARIA ADJUSTED: Forzando asimetria matemática. Nuevo lote: ", hedgeLot);
            }
            ...
            if(MarginOK(hedgeLot,hedgeType)){
               ...
               if(t1>0){
                  ...
                  m_stage3TriggerAtOpen=m_dyn.stage3Trigger;
                  Print("[AQ V7.9.3-XAU-STGPRE] >>> STAGE 2 via CONTRARIA DE FRENADO (lote ",DoubleToString(hedgeLot,2),"): #",t1);
               }
            }
         }
      }
   ...
   if(m_blockStage==2&&mainPosCount==2){
      ...
      double prevLot=MathMax(FindStageLotByComment("BSE_H1"),FindStageLotByComment("BSE_REINF1"));
      double thirdLot=CalcDirectionalLot(targetDir,prevLot);
      ...
      if(MarginOK(thirdLot,thirdType)){
         ...
         if(t2>0){
            ...
            Print("[AQ V7.9.3-XAU-STGPRE] >>> STAGE 3: 3ra DIRECCIONAL #",t2,
   ...
   if(m_blockStage==3){
      ...
      if(totalPnL<=trigger3){
         ...
         double prevLot4=MathMax(FindStageLotByComment("BSE_DIR_LONG"),FindStageLotByComment("BSE_DIR_SHORT"));
         double fourthLot=CalcDirectionalLot(target4Dir,prevLot4);
         ...
         if(MarginOK(fourthLot,fourthType)){
            ...
            if(t3>0){
               ...
               m_blockStage=4;
               Print("[AQ V7.9.3-XAU-STGPRE] >>> STAGE 4: Consolidacion #",t3,
...
void RunRecoveryEngine()
{
   ...
   if(!m_recoveryActive){
      ...
      Print("[AQ V7.9.3-XAU-STGPRE] RECOVERY FALLBACK | PnL=$",NormalizeDouble(m_port.totalProfit,2));
   }
...
void ActivateLBC()
{
   ...
   Print("[AQ V7.9.3-XAU-STGPRE] LBC ACTIVADO | LibreMarg=$",NormalizeDouble(freeMarg,2)," | MaxPares=",m_lbc.maxOrdersCalc);
}

void DeactivateLBC()
{
   if(!m_lbc.active) return;
   Print("[AQ V7.9.3-XAU-STGPRE] LBC DESACTIVADO | Cosechado:$",NormalizeDouble(m_lbc.harvestedTotal,2)," en ",m_lbc.harvestCount," cosechas");
   ZeroMemory(m_lbc);
}
...
void CheckCycleMaxLoss()
{
   ...
   if(m_port.totalProfit<=Inp_CycleMaxLossUSD){
      Print("[AQ V7.9.3-XAU-STGPRE] CYCLE MAX LOSS: $",NormalizeDouble(m_port.totalProfit,2));
...
bool CheckEquityGuard()
{
   ...
   if(m_port.totalProfit<=Inp_EmergencyLossUSD&&!m_emergencyMode){
      Print("[AQ V7.9.3-XAU-STGPRE] ALERTA EQUITY: $",NormalizeDouble(m_port.totalProfit,2)," -> Pausa primarias.");
...
void RunCTEngine()
{
   ...
   if(m_port.totalPos==0){
      if(!m_sensors.allOK){
         ...
         if(TimeCurrent()-lastSL>=60){Print("[AQ V7.9.3-XAU-STGPRE] ENTRADA BLOQUEADA: ",m_sensors.blockReason);lastSL=TimeCurrent();}
         return;
      }
      ...
      if(ticket>0){
         ...
         m_detangleDetectTime=0;m_detangleActive=false;
         DeactivateLBC();
         Print("[AQ V7.9.3-XAU-STGPRE] PRIMARY #",ticket," ",(initType==ORDER_TYPE_BUY?"BUY":"SELL"));
      }
      m_isProcessing=false;
      return;
   }
...
{
   // [V7.9.3-STAGE-PRECISION] Antes se ejecutaba en paralelo con el Block
   // Stage Engine y podia abrir su propia cobertura al mismo tiempo que
   // Stage1-4 gestionaba el grupo -- fuente de aperturas duplicadas.
   // Ahora solo actua como red de seguridad cuando el BSE NO esta activo
   // (posiciones huerfanas tras un reinicio, etc.)
   if(!Inp_UseNetHedge||m_port.totalPos==0||m_isProcessing||m_blockStage>0) return;
   ...
   if(loss<=m_dyn.netHedgeTrig2&&!m_netHedge2Applied){
      ...
      if(hedgeLot>0) {
         if(!AntiSymmetricOK(hedgeType, hedgeLot)) {
            hedgeLot = NormLot(hedgeLot + volStep);
            Print("[AQ V7.9.3-XAU-STGPRE] NET HEDGE L2 ADJUSTED to prevent 0.00 Net Volume.");
         }
         ...
         if(MarginOK_Hedge(hedgeLot,hedgeType)){
            ...
            if(ticket>0){m_netHedge2Applied=m_netHedge1Applied=true;m_lastNetHedgeTime=TimeCurrent();
               ...
               Print("[AQ V7.9.3-XAU-STGPRE] NET HEDGE L2 | PnL=$",NormalizeDouble(loss,2));}
         }
      }
      return;
   }
   ...
   if(loss<=m_dyn.netHedgeTrig1&&!m_netHedge1Applied){
      ...
      if(hedgeLot>0) {
         if(!AntiSymmetricOK(hedgeType, hedgeLot)) {
            hedgeLot = NormLot(hedgeLot + volStep);
            Print("[AQ V7.9.3-XAU-STGPRE] NET HEDGE L1 ADJUSTED to prevent 0.00 Net Volume.");
         }
         ...
         if(MarginOK_Hedge(hedgeLot,hedgeType)){
            ...
            if(ticket>0){m_netHedge1Applied=true;m_lastNetHedgeTime=TimeCurrent();
               ...
               Print("[AQ V7.9.3-XAU-STGPRE] NET HEDGE L1 | PnL=$",NormalizeDouble(loss,2));}
         }
      }
   }
}
...
void RunVolatilityStormFilter()
{
   ...
   if(stormNow&&!m_stormActive){m_stormActive=true;m_stormDetectedTime=TimeCurrent();Print("[AQ V7.9.3-XAU-STGPRE] TORMENTA | ATRx=",NormalizeDouble(m_stormLastATRRatio,2));}
   if(m_stormActive){if(TimeCurrent()-m_stormDetectedTime>=Inp_StormCooldownSec){if(!stormNow){m_stormActive=false;Print("[AQ V7.9.3-XAU-STGPRE] TORMENTA DESPEJADA");}else m_stormDetectedTime=TimeCurrent();}}
}
...
int OnInit()
{
   ...
   if(h_ATR==INVALID_HANDLE||h_EMAFast==INVALID_HANDLE||h_EMASlow==INVALID_HANDLE||h_RSI==INVALID_HANDLE||h_MACD==INVALID_HANDLE){Print("[AQ V7.9.3-XAU-STGPRE] ERROR: Indicadores base");return INIT_FAILED;}
   ...
   if(m_port.totalPos>0) Print("[AQ V7.9.3-XAU-STGPRE] Posiciones existentes (",m_port.totalPos,"): Stage fallback hasta nueva primaria.");
   ...
   Print("[AQ V7.9.3-XAU-STGPRE] LISTO | Saldo=$",m_initialBalance," | Marg0.01=$",NormalizeDouble(CalcMarginFor001(),2));
...
{
   Print("[AQ V7.9.3-XAU-STGPRE] DETENIDO | PnL=$",NormalizeDouble(m_totalPnL,2)," | Trades:",m_tradesClosed);
   ...
   if(id==CHARTEVENT_OBJECT_CLICK){
      if(sp=="AQ79_B1"){
         ...
         if(!m_isPaused){
            ...
            m_stage1TriggerAtOpen=m_stage3TriggerAtOpen=0;
            m_detangleDetectTime=0;m_detangleActive=false;m_primaryOpenTime=0;
            DeactivateLBC();Print("[AQ V7.9.3-XAU-STGPRE] SISTEMA REANUDADO");
         } else Print("[AQ V7.9.3-XAU-STGPRE] SISTEMA PAUSADO");
      }
      if(sp=="AQ79_B2"){
         Print("[AQ V7.9.3-XAU-STGPRE] CIERRE MANUAL...");int closed=0;
         m_isProcessing=true;
         for(int i=PositionsTotal()-1;i>=0;i--){
            ulong t=PositionGetTicket(i);if(!PositionSelectByTicket(t))continue;
            if(PositionGetString(POSITION_SYMBOL)!=_Symbol)continue;
         ...
         m_blockStage=0;m_stageFollowHedge=false;m_stage1TriggerAtOpen=m_stage3TriggerAtOpen=0;
         m_detangleDetectTime=0;m_detangleActive=false;m_primaryOpenTime=0;
         DeactivateLBC();Print("[AQ V7.9.3-XAU-STGPRE] CIERRE MANUAL: ",closed," posiciones cerradas");
      }
      ChartRedraw(0);
   }
}
