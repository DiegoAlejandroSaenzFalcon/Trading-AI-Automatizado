//+------------------------------------------------------------------+
//|   APEXQUANT - V7.9.1-ASYMMETRIC-BTC                              |
//|   "DIRECTIONAL RECOVERY — ANTI-SYMMETRIC ENGINE"                 |
//|                                                                  |
//| BASE: V7.8-DYNAMIC                                               |
//| ASSET TARGET: BTCUSD (Crypto 24/7 Adaptation)                    |
//|                                                                  |
//| ADAPTACIONES CRITICAS PARA BITCOIN:                              |
//|   - [BTC-1] Mercado 24/7 habilitado. Removido el bloqueo de      |
//|     fines de semana (Sabado/Domingo) en las sesiones.            |
//|   - [BTC-2] Tolerancia de Spread escalada de 35 a 2500 puntos    |
//|     para asimilar la estructura de cotizacion del Bitcoin.       |
//|   - [BTC-3] Thresholds minimos base en USD ampliados para        |
//|     soportar el ruido intradia (ruido blanco) de BTC.            |
//|   - [HOTFIX] AntiSymmetric Guard enforcing physical min biases   |
//|     mantenido de la V7.9.1 intacto.                              |
//+------------------------------------------------------------------+
#property copyright "ApexQuant V7.9.1-BTC | BTCUSD Anti-Symmetric Engine"
#property version   "7.91"
#property strict
#property description "BTCUSD | V7.9.1-ASYMMETRIC | Directional Recovery | Anti-Symmetric Guard"

#define VERSION_STR   "APEXQUANT_V7.9.1-BTC"
...
input double Inp_DetangleMinLoss     = -3.00; // Escalado para BTC
...
input double Inp_DynMaxStage1USD     = 10.00; // Escalado para BTC
input double Inp_DynMaxStage3USD     = 20.00; // Escalado para BTC
input double Inp_DynMaxTPUSD         = 5.00;  // Escalado para BTC
...
input double Inp_Stage1Trigger       = -1.50; // Ampliado para BTC
input double Inp_Stage3Trigger       = -3.00; // Ampliado para BTC
...
input int    Inp_StormSpreadWindow    = 1600;
input int    Inp_StormCooldownSec     = 30;

input group "=== [V7.6B] NET EXPOSURE HEDGE ==="
input bool   Inp_UseNetHedge          = true;
input double Inp_NetHedgeTrigger1USD  = -5.0; // Escalado BTC
input double Inp_NetHedgeTrigger2USD  = -8.0; // Escalado BTC
...
input double Inp_BlockTPTarget       = 2.00; // Ampliado BTC
...
input double Inp_RecoveryTriggerUSD  = -1.00; // Ampliado BTC
...
input int    Inp_CTFixedPoints       = 1000; // BTC Points
...
input double Inp_CTMaxSpreadPoints   = 1500; // BTC Spread Tolerancia
input double Inp_CTMaxSpreadOff      = 1500; // BTC Spread Tolerancia
...
input double Inp_DailyLossPct        = 30.0;
...
input double Inp_EmergencyLossUSD    = -15.0; // Escalado para BTC
...
input int    Inp_MaxSpread           = 1500; // Escalado para el spread de BTCUSD
...
input string Inp_StartTime           = "00:00"; // BTC 24/7
input string Inp_EndTime             = "23:59"; // BTC 24/7
...
ENUM_SESSION_STATE GetCurrentSession()
{
   ...
   // BTC: Eliminada la restriccion de fin de semana para permitir 24/7
   ...
   return SESSION_OFF; // Raro en cripto, pero mantiene la estrucura original
}
...
bool AntiSymmetricOK(ENUM_ORDER_TYPE type, double lot)
{
   ...
   // The minimum acceptable net exposure must be the broker's volume step/min
   ...
   Print("[AQ V7.9.1-BTC] ANTI-SYMMETRIC GUARD: apertura bloqueada - crea simetria | NetVol=",
...
{
   MqlDateTime dt; TimeToStruct(TimeCurrent(),dt);
   // BTC: Eliminado bloqueo fines de semana
...
bool DailyLimitReached()
{
   ...
   if(eff<=-lim){Print("[AQ V7.9.1-BTC] LIMITE DIARIO");m_dailyLimitHit=true;m_isPaused=true;}
...
bool ClosePos(ulong ticket, string reason="")
{
   ...
   if(!m_isProcessing&&m_port.totalPos>1){
      Print("[AQ V7.9.1-BTC] !! CIERRE INDIVIDUAL BLOQUEADO #",ticket," [",reason,"] totalPos=",m_port.totalPos);
      return false;
   }
   double pf=PositionGetDouble(POSITION_PROFIT)+PositionGetDouble(POSITION_SWAP);
   if(m_trade.PositionClose(ticket)){
      ...
      if(idx>=0){
         ...
         Print("[AQ V7.9.1-BTC] CERRADA #",ticket," $",NormalizeDouble(pf,2),(reason!=""?" ["+reason+"]":""));
         ZeroMemory(m_rec[idx]);
      }
      return true;
   }
...
bool CloseRescuePos(ulong ticket, string reason)
{
   ...
   if(m_trade.PositionClose(ticket)){m_totalPnL+=pf;m_tradesClosed++;
      Print("[AQ V7.9.1-BTC] RESCATE #",ticket," $",NormalizeDouble(pf,2)," [",reason,"]"); return true;}
   return false;
}
...
bool CloseBlockIfPositive(string reason)
{
   ...
   Print("[AQ V7.9.1-BTC] CIERRE POSITIVO: PnL=$",NormalizeDouble(m_port.totalProfit,2),
...
ulong OpenOrder(ENUM_ORDER_TYPE type,double lot,string comment,bool skipPosLimit=false)
{
   ...
   if(!ok){Print("[AQ V7.9.1-BTC] ERR apertura: ",m_trade.ResultRetcodeDescription());return 0;}
   ulong ticket=m_trade.ResultOrder();
   if(ticket>0){
      m_tradesOpened++;
      Print("[AQ V7.9.1-BTC] ABIERTA #",ticket," ",(type==ORDER_TYPE_BUY?"BUY":"SELL"),
...
void RunDetangle()
{
   ...
   if(m_detangleDetectTime==0){
      ...
      Print("[AQ V7.9.1-BTC] DETANGLE: jaula simetrica detectada | NetVol=",
   ...
   if(m_port.worstTicket>0&&m_port.worstProfit<0){
      Print("[AQ V7.9.1-BTC] DETANGLE EJECUTANDO: cerrando peor pos #",m_port.worstTicket,
         ...
         if(Inp_UseDirectionalStage&&trendDir==primaryDir&&trendDir!=0){
            ...
            if(MarginOK(reinLot,reinType)&&AntiSymmetricOK(reinType,reinLot)){
               ...
               if(t1>0){
                  ...
                  Print("[AQ V7.9.1-BTC] >>> STAGE 2 via REFUERZO...");
               }
            }
         } else {
            // HEDGE LOGIC
            ...
            if(!AntiSymmetricOK(hedgeType, hedgeLot)) {
                ...
                Print("[AQ V7.9.1-BTC] HEDGE ADJUSTED: Forzando asimetria matemática. Nuevo lote: ", hedgeLot);
            }
            ...
            if(MarginOK(hedgeLot,hedgeType)){
               ...
               if(t1>0){
                  ...
                  m_stage3TriggerAtOpen=m_dyn.stage3Trigger;
                  Print("[AQ V7.9.1-BTC] >>> STAGE 2 via HEDGE ASIMETRICO: H1 #",t1);
               }
            }
         }
      }
   ...
   if(m_blockStage==2&&mainPosCount==2){
      ...
      if(MarginOK(thirdLot,thirdType)){
         ...
         if(t2>0){
            ...
            Print("[AQ V7.9.1-BTC] >>> STAGE 3: 3ra DIRECCIONAL #",t2,
      ...
      if(totalPnL<=trigger3){
         ...
         if(MarginOK(fourthLot,fourthType)){
            ...
            if(t3>0){
               ...
               m_blockStage=4;
               Print("[AQ V7.9.1-BTC] >>> STAGE 4: Consolidacion #",t3,
...
void RunRecoveryEngine()
{
   ...
   if(!m_recoveryActive){
      ...
      Print("[AQ V7.9.1-BTC] RECOVERY FALLBACK | PnL=$",NormalizeDouble(m_port.totalProfit,2));
   }
...
void ActivateLBC()
{
   ...
   Print("[AQ V7.9.1-BTC] LBC ACTIVADO | LibreMarg=$",NormalizeDouble(freeMarg,2)," | MaxPares=",m_lbc.maxOrdersCalc);
}

void DeactivateLBC()
{
   if(!m_lbc.active) return;
   Print("[AQ V7.9.1-BTC] LBC DESACTIVADO | Cosechado:$",NormalizeDouble(m_lbc.harvestedTotal,2)," en ",m_lbc.harvestCount," cosechas");
   ZeroMemory(m_lbc);
}
...
void CheckCycleMaxLoss()
{
   ...
   if(m_port.totalProfit<=Inp_CycleMaxLossUSD){
      Print("[AQ V7.9.1-BTC] CYCLE MAX LOSS: $",NormalizeDouble(m_port.totalProfit,2));
...
bool CheckEquityGuard()
{
   ...
   if(m_port.totalProfit<=Inp_EmergencyLossUSD&&!m_emergencyMode){
      Print("[AQ V7.9.1-BTC] ALERTA EQUITY: $",NormalizeDouble(m_port.totalProfit,2)," -> Pausa primarias.");
...
void RunCTEngine()
{
   ...
   if(m_port.totalPos==0){
      if(!m_sensors.allOK){
         ...
         if(TimeCurrent()-lastSL>=60){Print("[AQ V7.9.1-BTC] ENTRADA BLOQUEADA: ",m_sensors.blockReason);lastSL=TimeCurrent();}
         return;
      }
      ...
      if(ticket>0){
         ...
         m_detangleDetectTime=0;m_detangleActive=false;
         DeactivateLBC();
         Print("[AQ V7.9.1-BTC] PRIMARY #",ticket," ",(initType==ORDER_TYPE_BUY?"BUY":"SELL"));
      }
      m_isProcessing=false;
      return;
   }
...
void RunNetExposureHedge()
{
   ...
   if(loss<=m_dyn.netHedgeTrig2&&!m_netHedge2Applied){
      ...
      if(hedgeLot>0) {
         if(!AntiSymmetricOK(hedgeType, hedgeLot)) {
            hedgeLot = NormLot(hedgeLot + volStep);
            Print("[AQ V7.9.1-BTC] NET HEDGE L2 ADJUSTED to prevent 0.00 Net Volume.");
         }
         ...
         if(MarginOK_Hedge(hedgeLot,hedgeType)){
            ...
            if(ticket>0){m_netHedge2Applied=m_netHedge1Applied=true;m_lastNetHedgeTime=TimeCurrent();
               ...
               Print("[AQ V7.9.1-BTC] NET HEDGE L2 | PnL=$",NormalizeDouble(loss,2));}
         }
      }
      return;
   }
   ...
   if(loss<=m_dyn.netHedgeTrig1&&!m_netHedge1Applied){
      ...
      if(hedgeLot>0) {
         if(!AntiSymmetricOK(hedgeType, hedgeLot)) {
            hedgeLot = NormLot(hedgeLot + volStep);
            Print("[AQ V7.9.1-BTC] NET HEDGE L1 ADJUSTED to prevent 0.00 Net Volume.");
         }
         ...
         if(MarginOK_Hedge(hedgeLot,hedgeType)){
            ...
            if(ticket>0){m_netHedge1Applied=true;m_lastNetHedgeTime=TimeCurrent();
               ...
               Print("[AQ V7.9.1-BTC] NET HEDGE L1 | PnL=$",NormalizeDouble(loss,2));}
         }
      }
   }
}
...
void RunVolatilityStormFilter()
{
   ...
   if(stormNow&&!m_stormActive){m_stormActive=true;m_stormDetectedTime=TimeCurrent();Print("[AQ V7.9.1-BTC] TORMENTA | ATRx=",NormalizeDouble(m_stormLastATRRatio,2));}
   if(m_stormActive){if(TimeCurrent()-m_stormDetectedTime>=Inp_StormCooldownSec){if(!stormNow){m_stormActive=false;Print("[AQ V7.9.1-BTC] TORMENTA DESPEJADA");}else m_stormDetectedTime=TimeCurrent();}}
}
...
{
   Print("==============================================================");
   Print("  "+VERSION_STR+" — DIRECTIONAL ASYMMETRIC RECOVERY (BTC)");
   Print("  [HOTFIX] AntiSymmetric Guard enforcing physical min biases.");
   Print("  [CRYPTO] 24/7 Enabled, MaxSpread adjusted to ",Inp_MaxSpread);
   ...
   if(h_ATR==INVALID_HANDLE||h_EMAFast==INVALID_HANDLE||h_EMASlow==INVALID_HANDLE||h_RSI==INVALID_HANDLE||h_MACD==INVALID_HANDLE){Print("[AQ V7.9.1-BTC] ERROR: Indicadores base");return INIT_FAILED;}
   ...
   if(m_port.totalPos>0) Print("[AQ V7.9.1-BTC] Posiciones existentes (",m_port.totalPos,"): Stage fallback hasta nueva primaria.");
   ...
   Print("[AQ V7.9.1-BTC] LISTO | Saldo=$",m_initialBalance," | Marg0.01=$",NormalizeDouble(CalcMarginFor001(),2));
...
{
   Print("[AQ V7.9.1-BTC] DETENIDO | PnL=$",NormalizeDouble(m_totalPnL,2)," | Trades:",m_tradesClosed);
   ...
   if(id==CHARTEVENT_OBJECT_CLICK){
      if(sp=="AQ79_B1"){
         ...
         if(!m_isPaused){
            ...
            m_stage1TriggerAtOpen=m_stage3TriggerAtOpen=0;
            m_detangleDetectTime=0;m_detangleActive=false;m_primaryOpenTime=0;
            DeactivateLBC();Print("[AQ V7.9.1-BTC] SISTEMA REANUDADO");
         } else Print("[AQ V7.9.1-BTC] SISTEMA PAUSADO");
      }
      if(sp=="AQ79_B2"){
         Print("[AQ V7.9.1-BTC] CIERRE MANUAL...");int closed=0;
         m_isProcessing=true;
         for(int i=PositionsTotal()-1;i>=0;i--){
            ulong t=PositionGetTicket(i);if(!PositionSelectByTicket(t))continue;
            if(PositionGetString(POSITION_SYMBOL)!=_Symbol)continue;
         ...
         m_blockStage=0;m_stageFollowHedge=false;m_stage1TriggerAtOpen=m_stage3TriggerAtOpen=0;
         m_detangleDetectTime=0;m_detangleActive=false;m_primaryOpenTime=0;
         DeactivateLBC();Print("[AQ V7.9.1-BTC] CIERRE MANUAL: ",closed," posiciones cerradas");
      }
      ChartRedraw(0);
   }
}
