...
def stats(trades, label):
    ...
    lots = [t["vol"] for t in trades]
    capped = sum(1 for v in lots if v >= max(lots)) # aprox
    print(f"{label}: pnl={tot:>9.1f}  win={w:>5.1f}%  PF={pf:>5.2f}  maxDD={maxdd:>8.1f} ({(maxdd/18811.97*100):.0f}%)  lote max={max(lots):.2f}  med={sum(lots)/len(lots):.2f}")

frm2 = dtm.datetime(2026, 5, 14); to2 = dtm.datetime(2026, 8, 11)
print("=== MAY-AGO: riesgo fino + cap de lote ===")
for r, cap in [(200, 2.0), (250, 2.0), (300, 2.0), (300, 4.0), (350, 4.0), (400, 4.0)]:
    t = run_period_equity(frm2, to2, r, cap, r*4)
    stats(t, f"riesgo ${r} cap {cap}")
mt5.shutdown()
