...
HSTART=15.0; HDATR=0.80; HMATR=1.20; MAXVOL=3.0; STOPUSD=-1500.0
USE_HEDGE=True
...
cycles=[]; stops=[]; curMaxNeg=0; recCount=0; hedgeCount=0; lastHedgePrice=0; lastHedgeDir=0; maxLots=0
        ...
        for p in list(positions):
            if p['sl']>0:
                if p['dir']==1 and row['low']<=p['sl']:
                    addbal("SL",p['lot']*(p['sl']-p['op'])*MPL); positions.remove(p); slHarvest+=1
                elif p['dir']==-1 and row['high']>=p['sl']:
                    addbal("SL",p['lot']*(p['op']-p['sl'])*MPL); positions.remove(p); slHarvest+=1
    prof=pnl_block() if positions else 0.0
    curMaxNeg=min(curMaxNeg,prof)
    if positions:
        # STOP DURO: si la perdida del bloque supera el limite -> cerrar todo
        if prof<=STOPUSD:
            addbal("STOP",prof)
            stops.append(dict(t=dtm.datetime.fromtimestamp(t),pnl=prof,orders=len(positions),maxneg=curMaxNeg))
...
n=len(cycles); ns=len(stops)
tp=sum(c['pnl'] for c in cycles if c['pnl']>0); tl=sum(-c['pnl'] for c in cycles if c['pnl']<0)
sls=sum(-s['pnl'] for s in stops)
print(f"=== V8+HEDGE ACOTADO | MAXVOL={MAXVOL} STOPUSD=${STOPUSD} | 01/03-11/08 ===")
print(f"Balance ${balance:.2f} | PnL ${balance-18873.64:+.2f} | ciclos {n} (wins {len([c for c in cycles if c['pnl']>0])}) | stops {ns} (-${sls:.0f})")
print(f"PF {tp/tl if tl else float('inf'):.2f} | PnL/mes ${(balance-18873.64)/5.35:+.2f} | MaxDD ${maxdd:.2f} ({maxdd/18873.64*100:.1f}%) | min eq ${mineq:.2f} {mineqT:%d/%m}")
print(f"Primarias {primCount} | SL harvest {slHarvest} | max vol {maxLots:.2f}")
...
if positions:
    ...
    pf=pnl_block()
    print(f"Bloque abierto final: {len(positions)} ord | neto {bv-sv:+.2f} | flot ${pf:+.2f}")
