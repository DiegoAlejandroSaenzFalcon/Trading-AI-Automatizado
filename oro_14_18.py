...
def stats(trades, label):
    ...
    print(f"{label}: trades={n:>3} pnl={tot:>9.1f}  win={w:>5.1f}%  PF={pf:>5.2f}  maxDD={maxdd:>8.1f} ({maxdd/18873.64*100:.0f}%)  lote med={sum(t['vol'] for t in trades)/n:.2f}")

frm1 = dtm.datetime(2026, 2, 1); to1 = dtm.datetime(2026, 5, 13)
frm2 = dtm.datetime(2026, 5, 14); to2 = dtm.datetime(2026, 8, 11)
print("=== ZONA ROBUSTA 14-18h: IS vs OOS (riesgo $20, cap 1.0, macro H1) ===")
for lbl, f, t in [("IS",frm1,to1),("OOS",frm2,to2)]:
    for h1_, h2_ in [(14,17),(15,18),(14,18),(15,17)]:
        tr = run_gold(f, t, 20, 1.0, h1_, h2_)
        stats(tr, f"{lbl} ventana {h1_:02d}-{h2_:02d}")
