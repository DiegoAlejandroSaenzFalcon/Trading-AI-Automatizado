...
void TryRecover()
{
   ...
   if(lastEntryPrice>0){
      ...
      // Recoveries: se abren cuando el precio se mueve EN CONTRA del bloque
      if(!adverse&&dist<minDist) return;
      if(!adverse&&dist>0) return; // precio a favor: no agregar, esperar cierre
   }
