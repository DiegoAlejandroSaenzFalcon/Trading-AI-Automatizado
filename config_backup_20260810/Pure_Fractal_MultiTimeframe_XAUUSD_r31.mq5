...
void TryEntries()
{
   ...
   for(int s = 0; s < MAX_SLOTS; s++)
   {
      ...
      bool ok = (sig == 1) ?
         g_trade.Buy(lots, _Symbol, 0.0, slPrice, tpPrice, "M31_S" + IntegerToString(s + 1)) :
         g_trade.Sell(lots, _Symbol, 0.0, slPrice, tpPrice, "M31_S" + IntegerToString(s + 1));
      if(ok)
      {
...
void SavePersistedState()
{
   ...
   GlobalVariableSet("M31_DAYSTART", (double)g_dayStart);
   GlobalVariableSet("M31_DAYPL", g_dayRealizedPL);
   GlobalVariableSet("M31_COOLDOWN", (double)g_cooldownUntil);
}
void LoadPersistedState()
{
   ...
   datetime dayStart = now - (dtNow.hour * 3600 + dtNow.min * 60 + dtNow.sec);
   if(GlobalVariableCheck("M31_COOLDOWN"))
   {
      datetime cd = (datetime)GlobalVariableGet("M31_COOLDOWN");
      if(cd > now) g_cooldownUntil = cd;
   }
   if(GlobalVariableCheck("M31_DAYSTART"))
   {
      datetime stored = (datetime)GlobalVariableGet("M31_DAYSTART");
      if(stored == dayStart)
      {
         g_dayStart = dayStart;
         g_dayRealizedPL = GlobalVariableGet("M31_DAYPL");
