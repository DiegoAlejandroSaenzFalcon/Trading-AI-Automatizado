...
def run(start,end,use_hedge,MAXVOL,STOPUSD,label,balance0=18873.64):
    ...
    MPL=100.0; sp=0.2602; margin1=120.0; HSTART=15.0; HDATR=0.80; HMATR=1.20
    ...
    cycles=0; stops_=0; winC=0; curMaxNeg=0; recCount=0; hedgeCount=0; lastHedgePrice=0; lastHedgeDir=0; maxLots=0
    eqpeak=balance0; maxdd=0; primCount=0; balance=balance0; slHarvest=0; maxRec=0; maxHg=0
    mineq=1e18; mineqT=None
    ...
    for i in range(WARMUP,len(df)):
        ...
        if positions:
            if prof<=STOPUSD:
                ...
                continue
            dyn=BTPUSD
            if atr>0: dyn=max(BTPUSD, (atr*BTPATR*MPL)*0.5+BTPUSD)
            if prof>=dyn:
                balance+=prof; cycles+=1
                if prof>0: winC+=1
                maxRec=max(maxRec,recCount); maxHg=max(maxHg,hedgeCount)
                positions.clear(); beLocked=False; trailActive=False; peakProfit=0; peakFav=0
                blockStart=t; lastPrim=t; recCount=0; hedgeCount=0; curMaxNeg=0; lastHedgePrice=0; lastHedgeDir=0
                continue
            ...
            hedgeActive = (prof < -HSTART) if use_hedge else False
            if prof<0 and hedgeActive and vol<MAXVOL:
                ...
                if moved>=atr*HDATR:
                    ...
                    if used/eq*100<MARGINPCT and free>=eq*0.01:
                        ...
                        hedgeCount+=1; lastRec=t; maxLots=max(maxLots,vol+lot)
    ...
    if positions:
        finalfl=pnl_block(); balance+=0
    else: finalfl=0
    print(f"{label}: PnL ${balance-balance0:+.0f} | ciclos {cycles} (win {100*winC/max(1,cycles):.0f}%) | stops {stops_} | harvest {slHarvest} | maxRec {maxRec} maxHg {maxHg} maxVol {maxLots:.1f} | MaxDD ${maxdd:.0f} ({maxdd/balance0*100:.0f}%) | minEq ${mineq:.0f} {mineqT:%d/%m} | finalOpen flot ${finalfl:+.0f} ({len(positions)} ord)")

run(dtm.datetime(2026,3,1),dtm.datetime(2026,8,11),False,10.0,-1e18,"A) V8 SIN hedge (ilimitado)",18873.64)
run(dtm.datetime(2026,3,1),dtm.datetime(2026,8,11),True,10.0,-1e18,"B) V8 + hedge (ilimitado)",18873.64)
run(dtm.datetime(2026,3,1),dtm.datetime(2026,8,11),True,3.0,-1500.0,"C) V8 + hedge acotado",18873.64)
run(dtm.datetime(2026,6,1),dtm.datetime(2026,8,1),True,3.0,-1500.0,"D) Mercado lateral jun-jul",18873.64)
run(dtm.datetime(2026,3,1),dtm.datetime(2026,8,11),False,3.0,-1500.0,"E) V8 SIN hedge acotado",18873.64)
