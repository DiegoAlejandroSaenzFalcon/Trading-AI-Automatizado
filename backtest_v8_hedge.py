...
HSTART=15.0; HDATR=0.80; HMATR=1.20; MAXVOL=10.0   # parametros hedge
...
eqpeak=18873.64; maxdd=0; primCount=0; balance=18873.64
...
for i in range(WARMUP,len(df)):
        ...
        for p in list(positions):
            if p['sl']>0:
                if p['dir']==1 and row['low']<=p['sl']:
                    balance+=p['lot']*(p['sl']-p['op']); positions.remove(p)
                elif p['dir']==-1 and row['high']>=p['sl']:
                    balance+=p['lot']*(p['op']-p['sl']); positions.remove(p)
    def pnl_block():
    ...
    if positions:
        ...
        # HEDGE: cuando la perdida es profunda, agrega en direccion opuesta al neto
        ...
        # RECOVERY (solo si el hedge NO esta activo)
    ...
    eq=balance+pnl_block(); eqpeak=max(eqpeak,eq); maxdd=max(maxdd,eqpeak-eq)

n=len(cycles)
tp=sum(c['pnl'] for c in cycles if c['pnl']>0); tl=sum(-c['pnl'] for c in cycles if c['pnl']<0)
print(f"=== V8 + HEDGE ANTI-SIMETRICO | 01/03-11/08 XAUUSDm (spread {sp}) ===")
print(f"HStart={HSTART} HDistATR={HDATR} HMoveATR={HMATR} MaxVol={MAXVOL}")
print(f"Balance ${balance:.2f} | PnL ${balance-18873.64:.2f} | ciclos {n} | wins {len([c for c in cycles if c['pnl']>0])} | PF {tp/tl if tl else float('inf'):.2f}")
print(f"MaxDD equity ${maxdd:.2f} ({maxdd/18873.64*100:.1f}%) | PnL/mes ${(balance-18873.64)/5.35:.2f}")
print(f"Primarias {primCount} | max recoveries/bloque {max([c['rec'] for c in cycles]+[0])} | max hedges/bloque {max([c['hedge'] for c in cycles]+[0])} | max volumen {maxLots:.2f}")
if positions:
    ...
    print(f"Bloque abierto final: {len(positions)} ord | vol {bv:.2f}L/{sv:.2f}S | neto {bv-sv:+.2f} | flotante ${pf:.2f}")
for c in sorted(cycles,key=lambda c:c['maxneg'])[:4]:
    print(f"  ciclo {c['t']:%d/%m}: pnl {c['pnl']:+.2f} | maxneg {c['maxneg']:+.2f} | {c['orders']} ord | {c['rec']} rec | {c['hedge']} hedge")
