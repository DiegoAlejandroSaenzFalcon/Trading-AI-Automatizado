...
input double InpRiskPerTradeUSD  = 30.0;  // Riesgo por operacion EN USD (ganador del barrido de sizing)
input double InpMaxLotSize       = 0.40;
...
input double InpMaxDailyLossUSD      = 120.0;  // Perdida REALIZADA maxima del dia EN USD (4x riesgo) -> bloquea nuevas entradas
...
double g_bufATR[];
input ulong  InpMagicBase = 30; // MAGIC UNICO de esta version (asignable en .set)
...
//===================================================================
string GV_DayStart(){ return("PFVG30B_"+_Symbol+"_"+IntegerToString(InpMagicBase)+"_dayStart"); }
string GV_DayPL()   { return("PFVG30B_"+_Symbol+"_"+IntegerToString(InpMagicBase)+"_dayPL"); }
string GV_Cooldown(){ return("PFVG30B_"+_Symbol+"_"+IntegerToString(InpMagicBase)+"_cooldown"); }
   ...
   if(goLong)
     {
      ...
      sent = trade.Buy(lots, _Symbol, ask, sl, tp, usedConfluence ? "R30B+FVG Buy" : "R30B Buy");
      entryPrice = ask;
     }
   else
     {
      ...
      sent = trade.Sell(lots, _Symbol, bid, sl, tp, usedConfluence ? "R30B+FVG Sell" : "R30B Sell");
      entryPrice = bid;
     }
