...
MPL=100.0; sp=0.2602; margin1=120.0
...
eqpeak=18873.64; maxdd=0; primCount=0; balance=18873.64; slHarvest=0; ledger={}
mineq=1e18; mineqT=None; maxnegfl=0; maxnegT=None; maxposfl=0

def addbal(why,amt):
    global balance
    balance+=amt
    ledger[why]=ledger.get(why,0)+amt
        ...
        for p in list(positions):
            if p['sl']>0:
                if p['dir']==1 and row['low']<=p['sl']:
                    addbal("SL_buy",p['lot']*(p['sl']-p['op'])*MPL); positions.remove(p); slHarvest+=1
                elif p['dir']==-1 and row['high']>=p['sl']:
                    addbal("SL_sell",p['lot']*(p['op']-p['sl'])*MPL); positions.remove(p); slHarvest+=1
    prof=pnl_block() if positions else 0.0
    curMaxNeg=min(curMaxNeg,prof)
    if prof<maxnegfl: maxnegfl=prof; maxnegT=dtm.datetime.fromtimestamp(t)
...
print(f"=== V8+HEDGE (CORREGIDO x100) | hedge={USE_HEDGE} | 01/03-11/08 XAUUSDm ===")
print(f"Balance ${balance:.2f} | PnL ${balance-18873.64:+.2f} | ciclos {n} | wins {len([c for c in cycles if c['pnl']>0])} | PF {tp/tl if tl else float('inf'):.2f} | PnL/mes ${(balance-18873.64)/5.35:+.2f}")
print(f"MaxDD ${maxdd:.2f} ({maxdd/18873.64*100:.1f}%) | Min equity ${mineq:.2f} {mineqT:%d/%m} | maxneg flot ${maxnegfl:+.2f} {maxnegT:%d/%m}")
print(f"Primarias {primCount} | SL harvest {slHarvest} | max rec {max([c['rec'] for c in cycles]+[0])} | max hedge {max([c['hedge'] for c in cycles]+[0])} | max vol {maxLots:.2f}")
print("Ledger:", {k: round(v,2) for k,v in sorted(ledger.items(), key=lambda x:-abs(x[1]))})
for p in positions:
    print(f"  {p['dir']:+d} lot={p['lot']:.2f} op={p['op']:.2f} sl={p['sl']:.2f} pnl={(bid-p['op'])*MPL*p['lot'] if p['dir']==1 else (p['op']-ask)*MPL*p['lot']:.2f} t={dtm.datetime.fromtimestamp(p['time']):%d/%m}")
print(f"  bid/ask={bid:.2f}/{ask:.2f} total={pnl_block():.2f}")
for c in sorted(cycles,key=lambda c:c['maxneg'])[:4]:
    print(f"  ciclo {c['t']:%d/%m}: {c['pnl']:+.2f} | maxneg {c['maxneg']:+.2f} | {c['orders']} ord | {c['rec']} rec | {c['hedge']} hg")
