...
# ajuste a oro: spread en unidades de precio (0.26) y riesgo por trade pequeno
br.AVG_SPREAD_EST = 0.26
br.SPREAD_HALF = 0.13

def run_gold(frm, to, risk_usd, max_lot, h1_, h2_, macro_h4=True):
    ...
    br.INP["MaxDailyLossUSD"] = risk_usd*4
    br.reset()
    ...
    h4 = mt5.copy_rates_range("XAUUSDm", mt5.TIMEFRAME_H4, frm, to)
    if macro_h4:
        macro = h4
    else:
        macro = h1  # sesgo macro en H1 (el oro es mas H1 que H4)
    ...
    for k in range(len(m1)):
        ...
        mac_bar_time = now - (now % (3600 if not macro_h4 else 14400))
...
def stats(trades, label):
    ...
    print(f"{label}: trades={n:>3} pnl={tot:>9.1f}  win={w:>5.1f}%  PF={pf:>5.2f}  maxDD={maxdd:>8.1f} ({maxdd/18873.64*100:.0f}%)")

frm1 = dtm.datetime(2026, 2, 1); to1 = dtm.datetime(2026, 5, 13)
print("=== ORO FVG: barrido de horas IS feb-may (riesgo $8, cap 0.5) ===")
for h1_, h2_ in [(12,16),(13,16),(14,16),(14,17),(15,18),(16,20),(13,17),(13,18),(12,15),(14,18)]:
    t = run_gold(frm1, to1, 8, 0.5, h1_, h2_)
    stats(t, f"ventana {h1_:02d}-{h2_:02d}")
print("=== ORO FVG: macro H1 vs H4 en ventana top ===")
for mac, lbl in [(True,"macro H4"),(False,"macro H1")]:
    t = run_gold(frm1, to1, 8, 0.5, 14, 16, mac)
    stats(t, f"14-16 {lbl}")
mt5.shutdown()
